...
public class HoldemPlayImpl implements HoldemPlay {
	...
	public void openCard(Scanner sc, Player p) {
		while (true) {
			...
			if (index >= 1 && index <= 3) {
				if (arrp[(index - 1)] == 0) {
					int open = p.getCards().get(index - 1);
					arrp[(index - 1)] = open;
					break;
				} else {
		...
		System.out.println(p.getName() + "님이 오픈한 카드");
		for (int c : arrp) {
			if (c == 0)
				continue;
			System.out.print(c + " ");
	...
	public void betting(Scanner sc, Player p) {
		System.out.println(p.getName() + "님의 배팅 순서입니다.");
		System.out.println("배팅 가능 금액 : " + (p.getPoint()-sumBet));
	...
	@Override
	public void result(HoldemPlayImpl hp, Player p, Player ai) {
		if (hp.checkRainbow(p) == true && hp.checkRainbow(ai) == true) {
			if (hp.checkSum(p) < hp.checkSum(ai)) {
				System.out.println("축하합니다. " + p.getName() + "님이 승리했습니다.");
				p.setPoint(p.getPoint() + sumBet);
				System.out.println("보유 포인트 : " + p.getPoint());
			} else {
				System.out.println("안타까워라. " + p.getName() + "님이 패배했습니다.");
				System.out.println("보유 포인트 : " + p.getPoint());
			}
