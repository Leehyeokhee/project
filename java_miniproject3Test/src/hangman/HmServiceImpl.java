...
public class HmServiceImpl implements HmService {
	...
	public void playGame(Player p) {
		...
		while(flag) {
			...
			switch(cate) {
			case "1":
				word=h.getQuizWordByCate("animal");
				flag = false;
				break;
			case "2":
				word=h.getQuizWordByCate("food");
				flag = false;
				break;
			default :
		...
		for(String w : quizWordList) System.out.print(w+" ");
		System.out.println("\n============================");
		...
		for(int i=0; i<quizWordList.size(); i++) {
			if(quizWordList.get(i)=="_") blankCnt++;
		}
			...
			if(wordList.contains(alpha)) {// 입력값이 맞을 경우
				for(int i=0; i<wordList.size(); i++) {
					if(wordList.get(i).equals(alpha) && quizWordList.get(i).equals("_")) {//빈칸이면서 i번째 글자랑 일치한다면
						...
						System.out.println("이미 입력한 문자입니다.");break;
					}
				}
			}
			else { //입력값이 틀리면
	...
	public void winGame(Player p) {
		...
		p.setPoint(p.getPoint()+h.getWinPoint());
		System.out.println("포인트를 "+h.getWinPoint()+" 얻었습니다. 현재 보유 포인트: "+p.getPoint());
	...
	public void getHint(Player p) {
		System.out.println("포인트가 "+h.getHintPrice()+" 차감됩니다. 힌트를 보시겠습니까? 현재 보유 포인트: "+p.getPoint());
		System.out.println("1. 힌트 보기 2. 문제로 되돌아 가기");
		int hint = sc.nextInt();
		
		switch(hint) {
		case 1: 
			if(p.getPoint() < h.getHintPrice()) {
				System.out.println("포인트가 부족합니다.");
				break;
			}else {
				p.setPoint(p.getPoint()-h.getHintPrice());
				System.out.println(h.getHintPrice()+"포인트가 차감되었습니다. 현재 보유 포인트: "+p.getPoint());
				for(int i=0; i<quizWordList.size(); i++) {
	...
	public void getUndo(Player p) {
		System.out.println("포인트가 "+h.getUndoPrice()+" 차감됩니다. 되돌리기를 사용하시겠습니까? 현재 보유 포인트: "+p.getPoint());
		System.out.println("1. 되돌리기 사용하기 2. 문제로 되돌아 가기");
		int hint = sc.nextInt();
		
		switch(hint) {
		case 1: 
			if(p.getPoint() < h.getUndoPrice()) {
				System.out.println("포인트가 부족합니다.");
				break;
			}else {
				p.setPoint(p.getPoint()-h.getUndoPrice());
				System.out.println(h.getUndoPrice()+"포인트가 차감되었습니다. 현재 보유 포인트: "+p.getPoint());
