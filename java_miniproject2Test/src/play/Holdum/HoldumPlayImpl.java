...
public class HoldumPlayImpl implements HoldumPlay {
	...
	@Override
	public void draw(ArrayList<Integer> deck, Player p1, HoldumPlayer ai) {
	...
	public void openCard(Scanner sc, Player p) {
		while (true) {
			...
			if (index >= 0 && index <= 2) {
				if (((HoldumPlayer) p).getArr()[index] == 0) {
					int open = ((HoldumPlayer) p).getCards().get(index);
					((HoldumPlayer) p).getArr()[index] = open;
					break;
				} else {
		...
		for (int c : ((HoldumPlayer) p).getArr()) {
			if (c == 0)
				continue;
			System.out.print(c + " ");
		}
	...
	public int checkSum(Player p1) {
		...
		for (int k : ((HoldumPlayer) p1).getCards()) {
			tmp1.add(k);
		}
	...
	public boolean checkRainbow(Player p1) {
		...
		for (int i : ((HoldumPlayer) p1).getCards()) {
			tmp.add(i);
		}
	...
	@Override
	public void result(HoldumPlayImpl hp, Player p1, HoldumPlayer ai) {
