...
public class HoldumPlayImpl implements HoldumPlay {
	...
	@Override
	public void draw(ArrayList<Integer> deck, HoldumPlayer p1, HoldumPlayer ai) {
	...
	@Override
	public void openCard(Scanner sc, HoldumPlayer p) {
		while (true) {
	...
	@Override
	public void openCard(HoldumPlayer p) {
		int index = 0;
	...
	@Override
	public void betting(Scanner sc, HoldumPlayer p) {
		System.out.println("Player의 배팅 순서입니다.");
		System.out.println("배팅 방식을 선택하세요");
		System.out.println("1. Call 2. Raise 3. All-in");
		int m = sc.nextInt();
		switch (m) {
		case 1:
	...
	@Override
	public void aiBetting(HoldumPlayer p) {
		...
		while (true) {
			if ((Math.random()) <= 0.91) {
				m = 1;
				break;
			} else if ((Math.random()) > 0.91 && (Math.random()) <= 0.995) {
				m = 2;
				break;
			} else {
				m = 3;
	...
	@Override
	public int checkSum(HoldumPlayer p1) {
	...
	@Override
	public boolean checkRainbow(HoldumPlayer p1) {
		boolean flag = false;
	...
	@Override
	public void result(HoldumPlayImpl hp, HoldumPlayer p1, HoldumPlayer ai) {
		if (hp.checkRainbow(p1) == true && hp.checkRainbow(ai) == true) {
			if (hp.checkSum(p1) < hp.checkSum(ai)) {
				...
				p1.setpoints(p1.getpoints() + sumBet);
				System.out.println("보유 포인트 : " + p1.getpoints());
			} else {
